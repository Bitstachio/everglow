...
This is the **extended description**: the squash commit message written when the PR is merged. The PR body that reviewers read while the PR is open is a different text with its own structure; see [pr-description-generator.md](./pr-description-generator.md).
