...
2. **Postgres** (application data): `User` row and related app data
...
JIT provisioning matters here: on first login, `UsersService.resolveByProviderSub` creates a Postgres `User` for an Auth0 `sub` if none exists. Any design that deletes Postgres while Auth0 still works can recreate the account on the next login.
...
- no Postgres `User` (and related data cleaned up according to product rules)
- no easy way for the same Auth0 identity to come back as a “new” app user via JIT

Auth0 and Postgres are separate systems. You can succeed in one and fail in the other. Crashes and timeouts happen in between. There is **no atomic commit** that covers both.
...
## 2. Why not one Postgres transaction around Auth0?

A common first instinct:

```text
BEGIN
  delete the Postgres user (and related rows)
...
**Postgres cannot commit Auth0.** The Management API call is an HTTP request to another service. Putting it inside an open DB transaction does not make Auth0 part of that transaction. At best you delay when Postgres becomes visible.
...
- Process crash or commit failure after Auth0 already deleted the identity still leaves **Auth0 gone and Postgres rolled back** (user row still there). You did not escape dual-store failure; you only changed which window you hit.

**Rollback cannot undo Auth0.** If Auth0 succeeded and then commit fails, “rollback the user” only restores Postgres. The IdP user is already gone.

Treat “wrap Auth0 in a DB transaction” as a false atomicity. Prefer a short Postgres transaction for local prep only, then call Auth0 outside it, then finish local delete, with a durable marker for recovery.

---

## 3. Ordering: Auth0 first vs Postgres first

Assume prep has reduced FK failures, but either step can still fail.

### Case A: Auth0 delete fails first

Postgres has not been deleted yet (or only a deletion flag was set and can be cleared).
...
### Case B: Auth0 succeeds, Postgres delete fails
...
| New Auth0 signup (new `sub`)        | New Postgres user; old row can become an orphan |
...
### Why we still prefer Auth0 before hard Postgres delete

Postgres-first (committed delete, then Auth0) has a sharp failure if Auth0 fails after the row is gone: the next login with the same Auth0 user **JIT-provisions a new `User`**. The account looks “undeleted.” That violates the intent of account deletion.
...
| ------------------------------------ | ----------------------- | ----------------------------------- |
| Postgres committed, then Auth0 fails | Row gone, Auth0 alive   | JIT recreates the account           |
| Auth0 gone, then Postgres fails      | Auth0 gone, row remains | Orphan app data; user may not retry |

We choose the second row’s risk, then mitigate it.

---

## 4. The approach: deletion flag + Auth0 + finish Postgres + reconciler

This is a small **saga / state machine** for dual-store delete. Durable intent lives in Postgres. Auth0 is an external step. A background job finishes what the request could not.
...
- In-flight JWTs may still work until they expire; that is acceptable for short-lived tokens. Flag checks on authenticated paths can tighten this later if needed.

### Why the flag matters

Without a flag, “Auth0 gone, Postgres row still here” is hard to find reliably. You would be guessing from Auth0 API scans or from failed requests that nobody can repeat.
...
Photo objects live in **S3** and metadata in **Postgres**. Event delete (and related flows) treat Postgres as the source of truth for “does this photo exist?”, remove rows first, then purge S3 best-effort. The photo orphan reconciler deletes S3 objects that no longer have a `Photo` row.
...
| ---------------------------------- | -------------------------------------------------------------------- | ----------------------------------------------------------- |
| Stores                             | Postgres + S3                                                        | Postgres + Auth0                                            |
| Canonical for “exists?” in the app | Postgres `Photo` row                                                 | App `User` row, but login is Auth0                          |
...
3. **Detectable, finishable** leftovers when Postgres teardown fails after Auth0.
