...
integration layer _which dependency_; only one layer emits the definitive `error` for a given failure chain.
...
- [ ] Do **not** log expected client errors (4xx) or re-log unhandled exceptions; ingress and the exception filter
      already do.
